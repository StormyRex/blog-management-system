...
;
    ...
    function renderCommentsEmptyState(message) {
        ...
        return $('<div>')
            ...
            .append(
                $('<i>')
                    .addClass('bi bi-chat-left-dots fs-1 text-secondary'),
    ...
    function getCommentProfileUrl(user) {
        ...
        return baseUrl + '/user/profile?id=' + userId;
    ...
    function renderCommentNode(comment, blogId, level) {
        ...
        var $content = $('<div>')
            .addClass('flex-grow-1 min-w-0');

        var $title = $('<div>')
            .addClass('d-flex flex-column gap-0 mb-1');

        var $name = createProfileLink(
            user,
            'fw-semibold text-body text-decoration-none'
        ).text(user.name || 'User');

        var $metaLine = $('<div>')
            .addClass('d-flex flex-wrap align-items-center gap-1 text-muted small');
        ...
        $content.append(
            $('<div>')
                .addClass('text-body mb-2')
                .text(comment.comment || '')
        );

        var $actions = $('<div>')
            .addClass('mt-1 d-flex align-items-center gap-3');

        $actions.append(
            renderCommentReplyButton(comment)
        );

        var $deleteButton = renderCommentDeleteButton(
            comment,
            blogId
        );
...
function renderComments(comments) {
    ...
    if (!comments || !comments.length) {
        ...
        $commentsList.append(
            renderCommentsEmptyState('Be the first to start the conversation.')
        );
    ...
    var blogId =
        $('#commentsModal').attr('data-blog-id') ||
        $('#commentsModal').data('blog-id');
    ...
    function deleteComment(commentId, blogId) {
        ...
        .done(function (res) {
            ...
            showCommentsModalAlert(
                res && res.message ? res.message : 'Unable to delete comment.'
            );
        ...
        .fail(function (xhr) {
            ...
            if (response && response.errors) {
                ...
                showCommentsModalAlert(
                    response.errors.id || response.message || 'Please fix the validation errors.'
                );
                return;
            ...
            showCommentsModalAlert(
                response && response.message ? response.message : 'Unable to delete comment.'
            );
    ...
    function postComment() {
        ...
        .done(function (res) {
            ...
            if (res && res.errors) {
                ...
                showCommentsModalAlert(
                    res.errors.comment || res.errors.blogId || res.message || 'Please fix the validation errors.'
                );
                return;
            ...
            showCommentsModalAlert(
                res && res.message ? res.message : 'Unable to create comment.'
            );
        ...
        .fail(function (xhr) {
            ...
            if (response && response.errors) {
                ...
                showCommentsModalAlert(
                    response.errors.comment || response.errors.blogId || response.message || 'Please fix the validation errors.'
                );
                return;
            ...
            showCommentsModalAlert(
                response && response.message ? response.message : 'Unable to create comment.'
            );
    ...
    function loadComments(blogId) {
        ...
        $commentsList
            .empty()
            .append(
                $('<div>').addClass('text-muted').text('Loading comments...')
            );
        ...
        .done(function (res) {
            ...
            if (res && res.success) {
                ...
                var count = res.data && typeof res.data.count !== 'undefined'
                    ? res.data.count
                    : comments.length;
            ...
            showCommentsEmptyState(
                res && res.message ? res.message : 'Unable to load comments.'
            );
